...
public class MainController {
    ...
    public String showLoginPage(
        ...
        model.addAllAttributes(createLoginAttributes(logout, error));
        return "login";
    }

    private Map<String, String> createLoginAttributes(Boolean logout, Boolean error) {
        Map<String, String> result = new HashMap<>();
        Optional.ofNullable(error).filter(Boolean::booleanValue)
                .ifPresent(e -> result.put("error", "Неверный логин или пароль"));
        Optional.ofNullable(logout).filter(Boolean::booleanValue)
                .ifPresent(l -> result.put("message", "Вы успешно вышли из системы"));
        return result;
    }
